...
class SentenceInvertedIndex:
    def __init__(self, index_path):
        schema = SentenceInvertedIndex.get_schema()
        tokenizer = SentenceInvertedIndex.get_tokenizer()
    ...
    def generate_index(all_cases, index_path):
        ...
        print(f"Starting Indexing for {len(all_cases)} documents (sentences)...")

        # Updated to unpack sentence_id
        for case_id, sentence_id, text in all_cases:
    ...
    def get_schema():
        ...
        # Updated the schema to use sentence_id
        schema_builder.add_text_field("sentence_id", stored=True)
    ...
    def search(self, query, top_k):
        ...
        for score, segment_address in results.hits:
            ...
            # Updated to pull sentence_id from the document hit
            sentence_id = doc["sentence_id"][0]
            tmp.append((case_id, sentence_id, score))
