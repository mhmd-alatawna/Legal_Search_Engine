...
def measure_metrics_at_k(searcher, test_cases, k, pool_size):
    ...
    def run_single_query(item):
        # Instantiate the new SentenceDatabase
        local_sentence_db = SentenceDatabase(DataLayerManager.sentences_DB)
        case_id, ground_truth_citations = item

        # Fetch query sentences
        query_sentences = local_sentence_db.get_sentences_by_case(case_id)
        ...
        # We return 3 values: Ground Truth count, Matches (True Positives), and Retrieved count
...
# Updated to use SentenceDatabase and case_to_sentence_count
sentence_db = SentenceDatabase(DataLayerManager.sentences_DB)
case_to_sentence_count = dict(sentence_db.get_case_to_sentence_count())

# Updated to use SentenceToSentenceSearcher
searcher = SentenceToSentenceSearcher(DataLayerManager.sentences_inverted_index_DB, case_to_sentence_count)
