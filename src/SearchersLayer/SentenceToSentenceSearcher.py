...
class SentenceToSentenceSearcher:
    ...
    def search(self, query_sentences, top_k=5, pool_size_per_query=100):
        """
        Executes a BM25 search for a full case (list of sentences).
        Aggregates the segment hits and returns the top-K scored documents.
        """
        ...
        for sent in query_sentences:
            ...
            for case_id, sentence_id, score in hits:
                key = (case_id, sentence_id)
                # Sum the score if multiple query sentences hit this same target sentence
        ...
        combined_segment_results = [
            (case_id, sentence_id, score)
            for (case_id, sentence_id), score in aggregated_segment_scores.items()
        ]
    ...
    def doc_to_macro_score(self, segment_search_results):
        ...
        # 1. SUM all the segment scores for each document
        for case_id, sentence_id, score in segment_search_results:
        ...
        for doc_id, raw_score in raw_document_scores.items():
            # Get how many total sentences this target document has
