...
class ParagraphToParagraphSearcher:
    ...
    def search(self, query_paragraphs, top_k=5, pool_size_per_query=100):
        """
        Executes a BM25 search for a full case (list of paragraphs).
        ...
        for para in query_paragraphs:
            ...
            for case_id, para_id, score in hits:
                key = (case_id, para_id)
                # Sum the score if multiple query paragraphs hit this same target paragraph
    ...
    def doc_to_macro_score(self, segment_search_results):
        ...
        # Apply threshold if needed (Optional)
        # if final_documents:
        #     thresh_score = self.thresh * final_documents[0][1]
        #     final_documents = [doc for doc in final_documents if doc[1] >= thresh_score]
